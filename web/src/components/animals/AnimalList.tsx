    ...
    <div className="bg-white dark:bg-gray-900 rounded-xl shadow-md p-6 mb-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-3">
          ...
          {(filters.species || filters.age || filters.size || filters.goodWith || filters.location || filters.price || searchTerm) && (
              <button
      ...
      {!showAdopted && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          {/* Good with filter */}
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Good With</label>
            <select
          ...
          {/* Price filter */}
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Adoption Fee</label>
            <select
              value={filters.price}
      ...
      {(filters.species || filters.age || filters.size || filters.goodWith || filters.location || filters.price || searchTerm) && (
        <div className="mt-6 pt-4 border-t border-gray-200 dark:border-gray-700">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {filters.age && (
              <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-gray-100 text-gray-800 dark:bg-gray-800 dark:text-gray-200">
                Age: {filters.age.charAt(0).toUpperCase() + filters.age.slice(1)}
                <button 
            ...
            >
              ...
              <div className="p-4 sm:p-6 flex-1 flex flex-col">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-2">
                  <div>
                    <div className="text-xs text-gray-500 dark:text-gray-400 mb-1">ID: {animal.uniqueId}</div>
                    <h3 className="text-xl font-bold text-gray-800 dark:text-white">{animal.name}</h3>
                  </div>
                  <div className="flex items-center gap-3 mt-2 sm:mt-0">
                    <span className={`text-sm font-medium px-2 py-0.5 rounded-full ${
                      animal.sex === 'Male' ? 'bg-blue-100 text-blue-800 dark:bg-blue-900 dark:text-blue-100' : 
                      'bg-pink-100 text-pink-800 dark:bg-pink-900 dark:text-pink-100'
                    }`}>
                      {animal.sex}
                    </span>
                    {!showAdopted && (
                      <span className="text-[#4C51A4] font-semibold">${animal.adoptionFee}</span>
                    )}
                    {showAdopted && animal.adoptionDate && (
                      <span className="text-green-500 font-semibold">Adopted: {animal.adoptionDate}</span>
                    )}
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-2 text-sm text-gray-600 dark:text-gray-400 mb-3">
                  ...
                  {!showAdopted && (
                    <div className="flex items-center">
                      ...
                      <span>{calculateDaysInSFP(animal.intakeDate)} days in SFP</span>
                    </div>
                  )}
                  <div className="flex items-center">
                ...
                <p className="text-gray-700 dark:text-gray-300 text-sm line-clamp-2 mb-4">
                  {showAdopted && animal.adoptionStory ? animal.adoptionStory : animal.description}
                </p>
